...
local function code_for_struct(st,fundefs,structs)
	...
	--local st2 = opaque_structs[st] and st.."_opq" or st
	local st2 = st
	table.insert(code,[[M.]]..st..[[ = ffi.metatype("]]..st2..[[",]]..st..")")
...
local function code_for_imguifuns(st,fundefs,structs)
	...
	for _,f in ipairs(funs) do
		...
		for _,def in ipairs(defs) do
		-- error"debug"
		-- if def.cimguiname:match"Language_C" then error"debug" end
		-- if def.ov_cimguiname:match"Language_C" then error"debug" end
		--print("fun",def.cimguiname, def.ov_cimguiname)
			--if st=="Language" then print(st,def.cimguiname) end
			--can be an opaque_struct static function
			if (not opaque_structs[st]) or def.is_static_function then
			--if st=="Language" then print(st,def.cimguiname,def.location) end
			if FREETYPE or (not def.location:match"imgui_freetype") then
				def.old_stname = def.stname
				def.stname = "M"
...
local function make_enums(sources)
	for i,src in ipairs(sources) do
		local standenu = dofile([[../]]..src..[[/generator/output/structs_and_enums.lua]])
		--first enums
		for k,enu in pairs(standenu.enums) do
			if enumnames[k] then 
				print("--repeated enum name "..enumnames[k].." "..k.." from:"..src)
				k = src.."_"..k
			end
			enumnames[k]= src
			enums_for_save[k] = {}
			for i,v in ipairs(enu) do
				assert(v.calc_value)
				assert(not enumsvalues[v.name])
				enumsvalues[v.name] = v.calc_value
				table.insert(enums_for_save[k], {name = v.name, value = v.calc_value})
			end
		end
		--check not repeated structs

		for k,st in pairs(standenu.structs) do
			assert(not all_structs[k], "repeated struct")
			assert(not enumnames[k], "enumname as struct name")
			all_structs[k] = st
		end
		--now opaque_structs
		assert(standenu.opaque_structs, src)
		for k,opq in pairs(standenu.opaque_structs) do
			opaque_structs[k] = opq
		end
		--now constants
		local ok,const = pcall(dofile,[[../]]..src..[[/generator/output/constants.lua]])
		--print("const",src,ok,const)
		if ok then
			for key,val in pairs(const) do
				if val:match"%b()" then val = "M."..val end
				assert(not constants[key], "repeated constant")
				constants[key] = val
			end
		end
	end
...
local function class_gen(sources, ft_gen)
	...
	cpp2ffi.table_do_sorted(structs, function(struct,funs)
		if struct ~= "ImGui" and struct ~= "ImVec2" and struct ~= "ImStrv" and struct ~= "" then
			--if not opaque_structs[struct] then
				table.insert(strout,code_for_struct(struct,fundefs, structs))
			-- else
				-- table.insert(strout,code_for_imguifuns(struct,fundefs, structs))
			-- end
		end
	end)
